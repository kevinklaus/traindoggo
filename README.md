...
A train journey planner built for dog-friendly travel in Europe.

- **Search stations** with autocomplete.
- **Compare journey options** with route details and timing.
- **View carriage layouts** in detailed views from vagonWEB.
- **Get train travel recommendations** on travel days, changes etc.
- **Content on dog friendly train travel** e.g. Night Trains, Destinations and general tips & tricks
...
`traindoggo` is a train journey planner designed specifically for travelers with dogs. The app helps users search for train routes across Germany, compare journey connection timelines and carriage layouts with dog comfort in mind.

## What the app does

- Provides real-time station autocomplete for departure and destination inputs.
- Displays detailed journey options as visual cards with connection durations, transfer details.
- Highlights dog ticketing rules for large dogs (requiring an extra ticket).
- Employs a secure backend proxy layer to bypass browser cross-origin blocks (`CORS`) on production servers, route data through a high-performance regional setup, and offers an offline mock engine for local testing.
...
### Mock Data Included (6 routes)
- Berlin Hbf, Wiesbaden Hbf.
