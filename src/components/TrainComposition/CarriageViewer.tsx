...
export function CarriageViewer({ url}: Props) {
//export function CarriageViewer({ url, title, hasPrev, hasNext, onPrev, onNext, onClose }: Props) {
  ...
  return (
    <div className="mb-4 border border-slate-300 text-center rounded-2xl overflow-hidden shrink-0 animate-in fade-in slide-in-from-top-2 relative">
{/*       <div className="py-2 px-2 flex items-center justify-between z-20 relative">
      ...
      <iframe 
        src={url} 
        className="w-full h-[200px] bg-white block relative z-0" 
            ...
            if (img && doc) {
              ...
              doc.body.innerHTML = `
                <style>
                  ...
                  img { display: block; height: 200px; width: auto; max-width: none !important; margin: 0; border-radius: 4px; }
                </style>
