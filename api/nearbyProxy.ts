...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!latitude || !longitude) return res.status(400).json({ error: 'Missing required positional data coordinates' });

  const targetUrl = `https://v6.db.transport.rest/locations/nearby?latitude=${latitude}&longitude=${longitude}&results=8`;
  console.log(`[PROXY NEARBY] Outgoing nearby fetch target: ${targetUrl}`);
